...
//			$.each(characters.match(/[a-z_+]{1}(\[[0-9a-z_+]{0,}(,[0-9a-z_+ ]+)?\])?/gi), function (column, characterParams) {
            $.each(characters.match(/./gi), function (column, characterParams) {
				var matches         = characterParams.match(/([a-z_+]{1})(\[([0-9a-z_+ ,]+)\])?/i),
					//no matter if user specifies [] params, the character should be in the second element
					character       = matches ? matches[1] : characterParams,
					//check if user has passed some additional params to override id or label
					params          = matches ? typeof matches[3] !== 'undefined' ? matches[3].split(',') : [] : null,
					//id param should be first
					overrideId      = params && params.length ? params[0] : null,
					//label param should be second
					overrideLabel   = params && params.length === 2 ? params[1] : null;
				if(character == '_'){
					$row.append($('<div></div>').addClass('seatCharts-cell seatCharts-space'));
				} else if (character == '+'){
					$row.append($('<div></div>').addClass('seatCharts-cell meetingroom-class'));
				} else if (character == '-'){
					$row.append($('<div></div>').addClass('seatCharts-cell aisle-class'));
				}
                else if(/[\u4e00-\u9fa5A-Z0-9]/.test(character)){
                    $row.append($('<div></div>').addClass('seatCharts-cell meetingroom-class').html(character));
                }
                else {
